...
export function MobileSmallInfo(props){
    const {bgColor=bgColor5e, headLine="Ausgaben", componentX="7%", componentY=50, componentWidth="300px", textLableTop=15, textLableLeft=20,
    ...
    return (
        <div style={{position:"relative"}}>
            ...
            <div style={{position:"relative", height:lableHight, width:componentWidth, left:componentX, top:(componentY+10), backgroundColor:bgColor, borderRadius:borderRadius}}>
                <TextLable id="MonthCostAvarge" top={textLableTop} left={textLableLeft} text={lable1Text} fontsize={fontSize} add={{whiteSpace:"pre-wrap", transform: "flex"}}/>
                <TextLable id="MonthCost" top={textLableTop * -1 * 1.8} left={textLableLeft + 150} text={lable2Text} fontsize={fontSize} add={{whiteSpace:"pre-wrap", transform: "flex"}}/>
            </div>
        </div>
    )
}